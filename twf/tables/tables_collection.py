...
class CollectionTable(tables.Table):
    ...
    def render_item_count(self, record):
        ...
        if count > 0:
            ...
            return format_html(
                ...
                '</div>',
                count,
                reviewed, open_items, faulty,
                reviewed_percent, reviewed, reviewed if reviewed > 0 else "",
                open_percent, open_items, open_items if open_items > 0 else "",
                faulty_percent, faulty, faulty if faulty > 0 else ""
            )
        return "0 items"
    ...
    def render_actions(self, record):
        ...
        delete_modal = format_html(
            ...
            'data-message="Are you sure you want to delete this collection? This will remove all collection items as well.">'
        ...
        return format_html(
            ...
            '{}'
            '</div>',
            view_url, edit_url, delete_modal
        )
...
class CollectionItemTable(tables.Table):
    ...
    def render_document_id(self, record, value):
        ...
        if not value:
            return "-"
        return format_html('<a href="{}" class="fw-bold">{}</a>', 
    ...
    def render_status(self, record):
        ...
        status_html = format_html('<span class="badge {}">{}</span>', 
                                 status_classes.get(record.status, 'bg-secondary'),
                                 record.status)
        
        # Add workflow indicator if item is reserved
        if record.is_reserved:
            status_html = format_html('{}<br><span class="badge bg-secondary mt-1">In Workflow</span>', status_html)
    ...
    def render_actions(self, record):
        ...
        has_annotations = record.document_configuration and 'annotations' in record.document_configuration and record.document_configuration['annotations']
        
        download_buttons = ''
        if has_annotations:
            txt_url = reverse_lazy("twf:collection_item_download_txt", kwargs={"pk": record.pk})
            json_url = reverse_lazy("twf:collection_item_download_json", kwargs={"pk": record.pk})
            download_buttons = format_html(
                '<div class="dropdown-divider"></div>'
                '<a class="dropdown-item" href="{}" target="_blank"><i class="fa fa-file-text-o"></i> Download TXT</a>'
                '<a class="dropdown-item" href="{}" target="_blank"><i class="fa fa-file-code-o"></i> Download JSON</a>',
                txt_url, json_url
            )
        ...
        status_links = format_html(
            ...
            reverse_lazy("twf:collection_item_status_reviewed", kwargs={"pk": record.pk}),
        ...
        return format_html(
            ...
            '</ul>'
            '</div>',
            view_url, edit_url, view_url, edit_url, download_buttons, status_links, delete_modal
        )
