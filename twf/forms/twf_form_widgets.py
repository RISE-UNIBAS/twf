...
class JSONAnnotationsWidget(forms.Widget):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        for index, annotation in enumerate(value):
            delete_annotation_url = reverse_lazy('twf:collection_item_delete_annotation',
                                                 kwargs={'pk': item_pk, 'index': index + 1})
            if 'redirect_view' in self.attrs:
                delete_annotation_url += f'?redirect_to_view={self.attrs["redirect_view"]}'

            split_item_url = reverse_lazy('twf:collection_item_split', kwargs={'pk': item_pk, 'index': index + 1})
            if 'redirect_view' in self.attrs:
    ...
    class Media:
        css = {
            'all': ('twf/css/json_annotations_widget.css',)
        }
