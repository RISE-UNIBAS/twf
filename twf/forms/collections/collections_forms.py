...
class CollectionCreateForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            Row(Column('structure_tag_filter', css_class='form-group col-6 mb-3'),
...
class CollectionItemForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            ...
            self.fields['document_configuration'].widget.attrs.update({'item_pk': str(self.instance.pk)})
    ...
    def get_option_a_button(self, view, label, color="dark"):
        if self.instance.pk:
            ...
            url = '#'
        return HTML(f'<a href="{url}" class="btn btn-{color}" style="width: 100%">{label}</a>')
...
class CollectionItemUpdateForm(CollectionItemForm):
    ...
    def get_right_part(self):
        status_span = '<span class="badge bg-%s">%s</span>' % (self.get_status_display(),
                                                               self.instance.status)

        if self.instance and self.instance.pk:
            copy_item_url = reverse_lazy('twf:collection_item_copy', kwargs={'pk': self.instance.pk})
            delete_item_url = reverse_lazy('twf:collection_item_delete', kwargs={'pk': self.instance.pk})
            go_back_url = reverse_lazy('twf:collection_item_view', kwargs={'pk': self.instance.pk})
            if self.instance.document:
                view_doc_url = reverse_lazy('twf:view_document', kwargs={'pk': self.instance.document.pk})
            else:
                view_doc_url = '#'
        else:
            copy_item_url = '#'
        ...
        right_part = [
            Row(
                Column(HTML('<strong>Collection Item Status</strong> '
                            f'(Current status: {status_span})'), css_class='form-group col-12 mb-3'),
                css_class='row form-row'
            ),
            Row(
                Column(self.get_option_info('Open is the default state for a collection item.'),
                       css_class='form-group col-6 mb-1'),
                Column(self.get_option_a_button(
                    'twf:collection_item_status_open',
                    'Set Status Open',
                    'info'), css_class='form-group col-6 mb-1'),
                css_class='row form-row'
            ),
            Row(
                Column(self.get_option_info('Reviewed is the state for a collection item that has been checked.'),
                       css_class='form-group col-6 mb-1'),
                Column(self.get_option_a_button(
                    'twf:collection_item_status_reviewed',
                    'Set Status Reviewed',
                    'success'), css_class='form-group col-6 mb-1'),
                css_class='row form-row'
            ),
            Row(
                Column(self.get_option_info(
                ...
                Column(self.get_option_a_button(
                    ...
                    'Set Status Faulty',
                    'warning'), css_class='form-group col-6 mb-3'),
                css_class='row form-row'
            ),
            Row(
                Column(HTML("<strong>Collection Item Options</strong>"), css_class='form-group col-12 mt-3 mb-0'),
                css_class='row form-row'
            ),
            Row(
                Column(self.get_option_info(
                    'Connection items are connected to a document item if possible. As long as all the annotations stem'
                    ' from the same document, the connections will be kept. This does not work for structure-tag-based '
                    'collection items.'),
                    css_class='form-group col-6 mb-3'),
                Column(HTML(f'<a href="{view_doc_url}" class="btn btn-dark" style="width: 100%">View Document</a>'),
                       css_class='form-group col-6 mb-3'),
                css_class='row form-row'
            ),
            Row(
                Column(self.get_option_info(
                    'You can create a copy of this item.'),
                    css_class='form-group col-6 mb-3'),
                Column(
                    HTML(f'<a href="{copy_item_url}" class="btn btn-dark" style="width: 100%">Copy This Item</a>'),
                    css_class='form-group col-6 mb-3'),
                css_class='row form-row'
            ),
            Row(
                Column(self.get_option_info(
                    'Deleting the item cannot be undone.'),
                    css_class='form-group col-6 mb-3'),
                Column(HTML(
            ...
            Row(
                Column(HTML("<strong>Notes on Item</strong>"), css_class='form-group col-12 mt-3 mb-0'),
                css_class='row form-row'
            ),
            Row(
            ...
            Row(
                Column(HTML("<strong>Save Item</strong>"), css_class='form-group col-12 mt-3 mb-0'),
                css_class='row form-row'
            ),
            Row(
                Column(
                    self.get_option_info('Save the collection item if you changed the title or the notes.'
                                         'Status and annotation changes are saved instantly.'),
                    css_class='form-group col-6 mb-3'
                ),
                Column(
                    Submit('submit', 'Save Collection Item', css_class='btn btn-dark', style='width: 100%'),
                    css_class='form-group col-6 mb-3'
                ),
                css_class='row form-row'
            ),
...
class CollectionItemReviewForm(CollectionItemForm):
    ...
    def get_right_part(self):
        if self.instance and self.instance.pk:
            copy_item_url = reverse_lazy('twf:collection_item_copy', kwargs={'pk': self.instance.pk})
            copy_item_url += f'?redirect_to_view=twf:collections_review'

            delete_item_url = reverse_lazy('twf:collection_item_delete', kwargs={'pk': self.instance.pk})
            delete_item_url += f'?redirect_to_view=twf:collections_review'
        else:
            copy_item_url = '#'
            delete_item_url = '#'
        ...
        right_part = [
            Row(
                Column(HTML("<strong>Collection Item Review Options</strong>"), css_class='form-group col-12 mt-3 mb-0'),
                css_class='row form-row'
            ),
            Row(
                Column(self.get_option_info(
            ...
            Row(
                Column(
                    self.get_option_info('Save changes to title and notes before updating annotations. '
                                         'Annotation changes are saved instantly. Title and notes also get saved '
                                         'when setting the status to reviewed or faulty.'),
                    css_class='form-group col-6 mb-3'
                ),
                Column(
                    Submit('submit-u', 'Save Item', css_class='btn btn-dark', style='width: 100%'),
                    css_class='form-group col-6 mb-3'
                ),
                css_class='row form-row'
            ),
            ...
            Row(
                Column(HTML("<strong>Save Item and continue Review Workflow</strong>"), css_class='form-group col-12 mt-3 mb-0'),
                css_class='row form-row'
            ),
            Row(
                Column(
                    self.get_option_info('Delete this item and continue with the next item.'),
                    css_class='form-group col-6 mb-3'
                ),
                Column(
                    Submit('submit-d', 'Delete This Item', css_class='btn btn-danger', style='width: 100%'),
                    css_class='form-group col-6 mb-3'
                ),
                css_class='row form-row'
            ),
            Row(
                Column(
                    self.get_option_info('Set this item as reviewed and continue with the next item.'),
                    css_class='form-group col-6 mb-3'
                ),
                Column(
                    Submit('submit-f', 'Mark as faulty', css_class='btn btn-warning', style='width: 100%'),
                    css_class='form-group col-6 mb-3'
                ),
                css_class='row form-row'
            ),
            Row(
                ...
                Column(
                    Submit('submit-r', 'Mark as reviewed', css_class='btn btn-success', style='width: 100%'),
                    css_class='form-group col-6 mb-3'
                ),
                css_class='row form-row'
            )
