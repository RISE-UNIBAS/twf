        ...
        if routine == "manual":
            ...
            for doc in all_documents:
                ...
                for page in doc.get_active_pages():
                    fill_collection_item(
                        item, page, skip_empty_types, structure_tag_filter_list
                    )
            ...
            for doc in all_documents:
                for page in doc.get_active_pages():
                    ...
                    item.title = (
                        f"Item in {doc.document_id} - Page {page.tk_page_number}"
                    )
                    fill_collection_item(
                        item, page, skip_empty_types, structure_tag_filter_list
                    )
            ...
            for tag in structure_tags:
                item = CollectionItem(
                    collection=self.object, document_configuration={"annotations": []}
                )
                    ...
                    for page in doc.get_active_pages():
                        ...
                        for annotation in annotations:
                            ...
                            if cleaned_annotation["type"] == tag:
                                item.document_configuration["annotations"].append(
                                    cleaned_annotation
                                )
...
class TWFCollectionsAddDocumentView(FormView, TWFCollectionsView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, "Document has been added to the collection successfully."
        )
...
class TWFCollectionsReviewView(FormView, TWFCollectionsView):
    ...
    def form_valid(self, form):
        ...
        if action_d:
            ...
            if check_permission(
                self.request.user, "collection.manage", self.next_item.id
            ):
                self.next_item.delete()
                messages.success(
                    self.request, "Collection item has been deleted successfully."
                )
            else:
                messages.error(
                    self.request,
                    "You do not have permission to delete this collection item.",
                )
        elif action_r or action_f:
            logger.info(
                "Updating collection item and continuing workflow: %s", self.next_item
            )
            ...
            if custom_fields:
                ...
                if workflow_data:
                    ...
                    self.next_item.document_configuration["workflow_review"].update(
                        workflow_data
                    )
            ...
            self.next_item.save()
            messages.success(
                self.request, "Collection item has been updated successfully."
            )
        ...
        if not self.workflow_active:
            ...
            if self.workflow:
                ...
                context["workflow_definition"] = (
                    self.get_project().get_workflow_definition("review_collection")
                )
