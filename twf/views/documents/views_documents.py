...
class TWFDocumentsOverviewView(TWFDocumentView):
    ...
    def get_context_data(self, **kwargs):
        ...
        avg_pages = documents.annotate(num_pages=Count("pages")).aggregate(
            Avg("num_pages")
        )
        ...
        ignored_pages = sum(
            doc.pages.filter(is_ignored=True).count() for doc in documents
        )
        ignored_percentage = (
            (ignored_pages / total_pages * 100) if total_pages > 0 else 0
        )
        ignored_documents_count = documents.filter(is_ignored=True).count()
        ignored_documents_percentage = (
            (ignored_documents_count / total_documents * 100)
        ...
        tag_types = (
            project_tags.values("variation_type")
...
class TWFDocumentsBrowseView(SingleTableView, FilterView, TWFDocumentView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if hasattr(self, "filterset") and self.filterset:
            logger.debug(
                f"Filtered document queryset count: {self.filterset.qs.count()}"
            )
...
class TWFDocumentsSearchView(FormView, TWFDocumentView):
    ...
    def search_documents(self, cleaned_data):
        """
        Search and filter documents based on multiple criteria.

        Args:
            cleaned_data: Cleaned form data containing search parameters

        Returns:
            tuple: (queryset of matching documents, dict of match reasons per document)
        """
        import re

        project = self.get_project()
        queryset = Document.objects.filter(project=project).prefetch_related(
            "pages", "pages__tags"
        )
        match_reasons = {}
            ...
            for doc in queryset:
                if title.lower() in (doc.title or "").lower():
                    match_reasons.setdefault(doc.id, []).append(
                        {"type": "info", "text": f"Title contains “{title}”"}
                    )
        ...
        if doc_id:
            ...
            for doc in queryset:
                if doc_id.lower() in doc.document_id.lower():
                    match_reasons.setdefault(doc.id, []).append(
                        {"type": "info", "text": f"ID contains “{doc_id}”"}
                    )
        ...
        if has_entries:
            queryset = queryset.filter(
                pages__tags__dictionary_entry__in=has_entries
            ).distinct()
            for doc in queryset:
                matched_entries = {
                    tag.dictionary_entry
                ...
                for entry in matched_entries:
                    match_reasons.setdefault(doc.id, []).append(
                        f"Referenced entry: “{entry.label}”"
                    )
        ...
        if has_tags:
            queryset = queryset.filter(
                pages__tags__variation__icontains=has_tags
            ).distinct()
            for doc in queryset:
                for page in doc.pages.all():
                    for tag in page.tags.all():
                        if has_tags.lower() in tag.variation.lower():
                            match_reasons.setdefault(doc.id, []).append(
                                {"type": "info", "text": f"Tag text: “{tag.variation}”"}
                            )
                            break
            ...
            for doc in queryset:
                ...
                for page in doc.pages.all():
                    ...
                    try:
                        if use_regex:
                            matches = list(
                                re.finditer(
                                    document_text, page_text, flags=re.IGNORECASE
                                )
                            )
                            match_count = len(matches)
                            if matches:
                                found_snippet = self.get_highlighted_snippet(
                                    page_text, document_text, use_regex
                                )
                        else:
                ...
                if page_matches:
                    ...
                    match_reasons.setdefault(doc.id, []).append(
                        {
                            "type": "info",
                            "text": f"Matched on {len(page_matches)} page(s): {page_str}, "
                ...
                for doc in queryset:
                    ...
                    if found:
                        ...
                        match_reasons.setdefault(doc.id, []).append(
                            {
                                "type": "info",
                                "text": f'Page {obj}.{key} matched "{val}"',
                            }
                        )
        ...
        if cleaned_data.get("created_by"):
            ...
            for doc in queryset:
                if doc.created_by == cleaned_data["created_by"]:
                    match_reasons.setdefault(doc.id, []).append(
                        {"type": "info", "text": f"Created by: {doc.created_by}"}
                    )
        ...
        if cleaned_data.get("modified_by"):
            ...
            for doc in queryset:
                if doc.modified_by == cleaned_data["modified_by"]:
                    match_reasons.setdefault(doc.id, []).append(
                        {"type": "info", "text": f"Modified by: {doc.modified_by}"}
                    )
    ...
    def get_context_data(self, **kwargs):
        ...
        if "results" in kwargs:
            ...
            stats = {
                ...
                "with_tags": sum(
                    1
                    for doc in results
                    if any(page.tags.exists() for page in doc.pages.all())
                ),
                ...
                "needs_work": sum(
                    1 for doc in results if doc.status == "needs_tk_work"
                ),
        ...
        if (
            ...
            and self.request.POST.get("search_submitted") == "1"
        ):
            is_search = True
    ...
    def get_highlighted_snippet(text, pattern, use_regex=False, context_chars=30):
        """
        Extract and highlight a text snippet containing the search pattern.

        Args:
            text: Full text to search within
            pattern: Search pattern (string or regex)
            use_regex: Whether to treat pattern as regex (default: False)
            context_chars: Number of characters to show before/after match (default: 30)

        Returns:
            str: HTML snippet with highlighted match, or None if no match found
        """
        import re
...
class TWFDocumentDetailView(TWFDocumentView):
    ...
    def get_breadcrumbs(self):
        ...
        breadcrumbs.insert(
            -1, {"url": reverse("twf:documents_browse"), "value": "Browse Documents"}
        )
        return breadcrumbs
...
class TWFDocumentReviewView(TWFDocumentView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not workflow:
            ...
            context["workflow_definition"] = self.get_project().get_workflow_definition(
                "review_documents"
            )
    ...
    def post(self, request, *args, **kwargs):
        """
        Handle document review workflow actions.

        Args:
            request: Django HTTP request with document_id and action
            *args: Variable length argument list
            **kwargs: Arbitrary keyword arguments

        Returns:
            HttpResponse: Redirect to appropriate page based on action
        """
        workflow = (
            Workflow.objects.filter(
                project=self.get_project(),
        ...
        if document_id and action:
            ...
            if document:
                ...
                if action == "set_reviewed":
                    ...
                    irrelevant_note = (
                        "Marked as irrelevant - needs Exclude label in Transkribus"
                    )
