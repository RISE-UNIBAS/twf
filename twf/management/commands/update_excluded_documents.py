...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for doc in Document.objects.all():
            ...
            has_exclude = any(
                label.get("name", "").lower() == "exclude" for label in labels
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Documents {"would be " if dry_run else ""}updated: {documents_updated}'
            )
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Pages {"would be " if dry_run else ""}updated: {pages_updated}'
            )
        )
